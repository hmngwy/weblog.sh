  ...
  client.on('ready', function(ctx) {
    ...
    client.once('session', function(accept, reject) {
      ...
      session.once('exec', function(accept, reject, info) {
        ...
        // password
        // TODO
        // create/update article, handles push
        //   save -a 10 -s draft|public -b:body
        // delete article
        ...
        if(next === 'auth') {
          ...
          if(command[0] === 'key') {
            ...
            Article
            .find({author: user._id, status: payload})
            ...
            .exec(function(err, articles){
              ...
              if (articles.length != 0) {
                ...
                output.push("");
                output.push("");
                output.push("[" + user.username + "] → browse:" + payload);
                output.push("");
              ...
              if (articles.length == 0) {
                var response = [""];
                response.push("[" + user.username + "] → browse:" + payload );
                response.push("\n  No articles found.");
                response.push("\n");
            ...
            user.save(function(err, saved){
              ...
              var response = [""];
              response.push("[" + saved.username + "] → password updated");
          ...
          else if(command[0]==='scp') {
            console.log(info);

            stream.on('data', function(a,b,c){
              console.log(a,b,c);
            });
          }
          else {
