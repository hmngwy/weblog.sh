...
var saveArticle = function(opts, success, fail){
  ...
  var ext = opts.filename.substring(opts.filename.lastIndexOf('.')+1);

  // if .txt
  if(ext === 'txt' || ext === opts.filename) {
    ...
    opts.payload = markdown.render(opts.payload);

    var dom = cheerio.load(opts.payload);
    var title = dom('h1').text();

    dom('h1').remove(); // now remove it

    opts.payload = dom.html().trim();

  } // endif .md

  console.log(' **  saving:', title, opts.status, opts.filename);
  ...
  return function(accept, reject, info) {
    ...
    if(userMeta.next === 'auth') {
      ...
      if(command[0] === 'key') {
        ...
        var status = 'draft';
        if(parts && parts[1] === 'draft') status = 'draft';
